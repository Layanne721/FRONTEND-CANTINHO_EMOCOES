...
const diaSemanaHoje = computed(() => {
    const dia = new Date().getDay(); // 0=Domingo, 1=Segunda, etc.
    const map = { 1: 'segunda', 2: 'terca', 3: 'quarta', 4: 'quinta', 5: 'sexta' };
    return map[dia] || 'fim_de_semana';
});

// Lógica para filtrar objetivos por dia
function getObjetivosDoDia(diaKey) {
    ...
    // Filtra os objetivos que pertencem ao dia específico
...
const conteudoHoje = computed(() => {
    ...
    return {
        ...
        objetivos: getObjetivosDoDia(diaKey) // Inclui os objetivos do dia atual
    };
});
...
async function carregarDados() {
  ...
  try {
    ...
    if (semanarioAtual.value && semanarioAtual.value.objetivos) {
        try {
            semanarioAtual.value.objetivos = JSON.parse(semanarioAtual.value.objetivos);
        } catch (e) {
            semanarioAtual.value.objetivos = [];
        }
    }
    ...
    // --- DICA BASEADA NO RENDIMENTO ---
    const qtdPendentes = atividadesPendentes.value.length;

    if (qtdPendentes === 0) {
        dicaDoDia.value = { 
            titulo: 'Parabéns, tudo em dia!', 
            texto: 'Você é incrível! Todas as suas tarefas estão completas. Aproveite para brincar ou fazer um desenho livre! 🌟', 
        ...
        dicaDoDia.value = { 
            titulo: 'Falta pouco!', 
            texto: `Você tem apenas ${qtdPendentes} tarefinha(s) para fazer. Que tal terminar agora e ficar livre? 🚀`, 
            cor: 'bg-blue-50 text-blue-700 border-blue-200', 
            icon: '⏳' 
        };
    } else {
        dicaDoDia.value = { 
            titulo: 'Vamos nos organizar?', 
            texto: `Você tem ${qtdPendentes} atividades esperando. Respire fundo e faça uma de cada vez. Você consegue! 💪`, 
    ...
    <div class="w-full px-4 mt-6 flex flex-col items-center gap-6 max-w-4xl relative z-10 animate-fade-in">
                    ...
                    <div v-if="conteudoHoje.objetivos && conteudoHoje.objetivos.length > 0">
                        ...
                        <ul class="space-y-2">
                            <li v-for="(obj, idx) in conteudoHoje.objetivos" :key="idx" class="flex items-start gap-2 bg-indigo-50 p-2 rounded-xl border border-indigo-100">
                                ...
                                <div>
                                    <span class="block text-[10px] font-black uppercase text-indigo-400">{{ obj.categoria }}</span>
                                    <span class="text-xs font-bold text-gray-700 leading-tight">{{ obj.descricao }}</span>
                                </div>
                            </li>
                        </ul>
                    </div>
                ...
                <div v-else class="space-y-6 animate-fade-in">
                    <div v-for="(dia, key) in {segunda: 'Segunda', terca: 'Terça', quarta: 'Quarta', quinta: 'Quinta', sexta: 'Sexta'}" :key="key">
                        <div class="flex items-center gap-2 mb-1">
                            <h5 class="text-xs font-black text-indigo-400 uppercase">{{ dia }}</h5>
                            <span v-if="getObjetivosDoDia(key).length > 0" class="bg-indigo-100 text-indigo-600 text-[9px] px-1.5 py-0.5 rounded-md font-bold">{{ getObjetivosDoDia(key).length }} obj.</span>
                        </div>
                        
                        <p class="text-sm font-bold text-gray-600 bg-gray-50 p-3 rounded-xl border border-gray-100 whitespace-pre-line">
                            {{ semanarioAtual[key] || '---' }}
                        </p>

                        <div v-if="getObjetivosDoDia(key).length > 0" class="mt-2 pl-2 border-l-2 border-indigo-200 ml-1">
                            <div v-for="(obj, idx) in getObjetivosDoDia(key)" :key="idx" class="text-[10px] text-gray-500 font-bold mb-1">
                                • {{ obj.descricao }}
                            </div>
                        </div>
                    </div>
        ...
        <div v-if="dicaDoDia" class="w-full">
            <div :class="['p-4 rounded-3xl border flex items-center gap-4 shadow-sm transition-all', dicaDoDia.cor]">
                ...
                <div class="flex-1">
                    <h3 class="font-extrabold text-sm md:text-base uppercase opacity-80 mb-1">{{ dicaDoDia.titulo }}</h3>
                    <p class="text-sm md:text-base font-bold leading-snug">{{ dicaDoDia.texto }}</p>
                </div>
            </div>
        </div>
        ...
        <div v-if="atividadesPendentes.length > 0" class="w-full min-h-[80px]">
            ...
            <div class="space-y-3">
                <div v-for="(tarefa, index) in listaPendenciasExibida" :key="index" 
                     @click="irParaDesenho(tarefa)"
                     :class="['w-full bg-white border-2 rounded-2xl p-1 shadow-md cursor-pointer hover:scale-[1.01] transition-transform', index === 0 && !mostrarTodasPendencias ? 'border-red-400 ring-2 ring-red-100' : 'border-orange-200 opacity-90']">
                    
                    <div :class="['rounded-xl p-4 flex items-center gap-4', index === 0 && !mostrarTodasPendencias ? 'bg-red-50' : 'bg-orange-50']">
                        <div class="bg-white p-3 rounded-full text-2xl shadow-sm border border-gray-100">
                            {{ index === 0 && !mostrarTodasPendencias ? '🔥' : '📩' }}
                        </div>
                        <div class="flex-1">
                            <h3 :class="['font-black text-sm uppercase', index === 0 && !mostrarTodasPendencias ? 'text-red-500' : 'text-orange-400']">
                                {{ index === 0 && !mostrarTodasPendencias ? 'Fazer Agora' : 'Pendente' }}
                            </h3>
                            <p class="text-gray-600 font-bold text-xs md:text-sm">
                                Atividade: <span class="font-black text-gray-800">{{ tarefa.tipo }} {{ tarefa.conteudo }}</span>
                            </p>
                        </div>
                ...
                <div v-if="atividadesPendentes.length > 3" class="text-center pt-2">
                    <button @click="mostrarTodasPendencias = !mostrarTodasPendencias" class="text-sm font-bold text-indigo-500 hover:text-indigo-700 underline">
                        {{ mostrarTodasPendencias ? 'Ver menos' : `Ver mais ${atividadesPendentes.length - 3} tarefas...` }}
                    </button>
                </div>
            </div>
        </div>
        
        <div v-else-if="carregando" class="text-center py-6 text-gray-400 text-sm font-bold animate-pulse w-full">
            Verificando tarefas...
        </div>
